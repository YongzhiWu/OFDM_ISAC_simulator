...
SNRdBList = 0:3:30;
nFrame = 4000;
berList = zeros(size(SNRdBList));
for index = 1:length(SNRdBList)
    ber = zeros(1, nFrame);
    parfor t = 1:nFrame
        % Transmit data
        ...
        ComSNRdB = SNRdBList(index); % SNR of communication channel
        ...
        % disp(comResult);
        ber(t) = errorCount / (M * N * bitsPerSymbol);
    end
    berList(index) = mean(ber);
end
toc;

figure();
semilogy(SNRdBList, berList);
xlabel('SNR [dB]');
ylabel('BER');
grid on;
...
SNRdBList = -30:5:10;
nFrame = 1000;
rmseList = zeros(size(SNRdBList));
for index = 1:length(SNRdBList)
    rmse = zeros(1, nFrame);
    RadarSNRdB = SNRdBList(index); % SNR of radar sensing channel
    parfor t = 1:nFrame
        ...
        % disp(sensingResult);
        rmse(t) = mse(distanceE, target_pos);
    end
    rmseList(index) = sqrt(mean(rmse));
end

toc;

figure();
semilogy(SNRdBList, rmseList);
xlabel('SNR [dB]');
ylabel('RMSE [m]');
grid on;
